...
<div class="flex items-center justify-between mb-8 flex-wrap gap-4">
    <div>
        ...
        <p class="text-gray-500 text-sm">Kelola daftar keahlian Anda.</p>
    </div>
...
<?php if (isset($_GET['added'])): ?>
    <div class="mb-6 px-4 py-3 rounded-lg bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-sm">Skill berhasil ditambahkan.</div>
<?php endif; ?>
<?php if (isset($_GET['updated'])): ?>
    <div class="mb-6 px-4 py-3 rounded-lg bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-sm">Skill berhasil diperbarui.</div>
<?php endif; ?>
<?php if (isset($_GET['deleted'])): ?>
    <div class="mb-6 px-4 py-3 rounded-lg bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-sm">Skill berhasil dihapus.</div>
<?php endif; ?>
...
<div class="glass-card rounded-2xl overflow-hidden">
    <?php if (empty($skills)): ?>
        <div class="p-10 text-center">
            ...
            <a href="skill_form" class="text-neon-blue hover:text-cyan-400 transition-colors">Tambah skill pertama &rarr;</a>
        </div>
    <?php else: ?>
                        ...
                        <tr class="border-b border-neon-blue/5 table-row">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <a href="#" onclick="confirmDelete(<?= (int)$s['id'] ?>, '<?= htmlspecialchars(addslashes($s['name'])) ?>')" class="px-3 py-1.5 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20 transition-colors text-xs font-medium">Hapus</a>
                                </div>
                            </td>
                        </tr>
                    <?php endforeach; ?>
...
function confirmDelete(id, name) {
    if (confirm('Yakin ingin menghapus skill "' + name + '"? Aksi ini tidak bisa dibatalkan.')) {
